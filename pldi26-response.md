...
On our machine, using clang++ with default build options (-O3), this benchmark exactly unchanged takes 6.5s to compute fib(42). This version has _no_ cutoff, so its performance is dominated by the scheduler overhead with very small tasks. A Parabs implementation with no cutoff takes 10.2s. Both use the default settings of using as many domains/threads as possible.

We modified the taskflow example to use a cutoff value provided as an extra parameter, with cutoff=20 the Taskflow implementation takes 185ms (±12ms) and our Parabs version takes 346ms (±16ms), so it is around 1.8x slower. When configured to use a single domain/thread, the Taskflow version takes 1.5s and our Parabs version takes 2.3s. (On this benchmark we observe a parallel speedup of 8x for the Taskflow version and 6.6x for our Parabs version.)

Finally we studied the performance difference between C++ and OCaml on the sequential version (which is used below the cutoff). On our machine, the purely-sequential version of fibonacci (for fib(40)) takes 450ms with `clang++ -O2`, and 960ms with `ocamlopt` (only 880s with the OCaml 4.14 pre-multicore runtime, which does not need a resizable-stack check in function prologues). Clang uses an accumulator-passing-style transformation to make the `fib(n-2)` call tail-recursive. We applied this transformation manually on the OCaml source, and the resulting function takes around 510ms for fib(40). (Note: `g++ -O2` performs unspeakable unrolling optimizations and computes fib(40) in 250ms, we did not investigate further and stick to clang for more comparable results.)

When using this hand-optimized OCaml sequential implementation of `fib` below the cutoff, the Parabs version now takes 1.7s with a single thread (vs. 1.5s for Taskflow) and 246ms with 12 threads (vs. 185ms for Taskflow).
