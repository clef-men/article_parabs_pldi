...
.
Lightweight tasks and their scheduler are left to be implemented in userland, as OCaml libraries running on top of runtime domains.
...
Those schedulers have been used in performance-sensitive scenarios, benchmarked and optimized, notably using lock-free data structures implemented in \OCamlFive.
...
\begin{enumerate}
  \item
    ...
    In particular, our invariant lets us reason about the failure case of the \c{pop} operation, which was missing from earlier formalizations and is essential to prove completion of the work-stealing scheduler when all task queues are exhausted.
  \item
...
% Line count numbers:
%   git checkout pldi26-artifact
%   # count
%   find lib/examples lib/zoo_parabs lib/zoo_saturn/ws_* lib/zoo_saturn/mpmc_queue_* lib/zoo_std/mpsc_* lib/zoo_std/random* lib/zoo_std/queue* lib/zoo_std/ivar* -name '*.ml' | xargs wc -l
%   = 2Kloc
%   # proofs
%   find theories/examples theories/zoo_parabs theories/zoo_saturn/ws_* theories/zoo_saturn/mpmc_queue_* theories/zoo_std/mpsc_* theories/zoo_std/random* theories/zoo_std/queue* theories/zoo_std/ivar* -name '*.v' | grep -v "__" | xargs wc -l
%   + 1K lines for arrays
%   = 26Kloc
