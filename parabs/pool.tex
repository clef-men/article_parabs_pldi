...
(2) Since \ZooLang does not support algebraic effects~\citep{DBLP:conf/pldi/Sivaramakrishnan21} neither, the interface is slightly more involved (see \emph{execution contexts} in \cref{sec:parabs_pool_spec}) and the implementation of futures (see \cref{sec:parabs_pool_spec}), especially the \c{wait} function, is less efficient (see \cref{sec:parabs_pool_impl}).
