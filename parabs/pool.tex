...
\item[\ocamlinline{val size : context -> int}] returns the number of
  ...
  \Xgabriel{As I was writing this I wondered whether we really want to
    expose \ocamlinline{size} in the paper. Could we drop it from the
    API, from the specification, and as an argument of the
    $\mathsf{inv}$ predicate in the spec?}
...
\end{ocamlcode}
Because \ocamlinline{Pool.async} does not help waiting on a result, we
use an atomic reference to store the result and wait on it. This
pattern is wrapped in an \ocamlinline{Ivar} module from the Zoo standard
library, but it is even better to use \ocamlinline{Future} directly.
