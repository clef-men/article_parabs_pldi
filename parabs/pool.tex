...
\item[\ocamlinline{val run : t -> 'a task -> 'a}] runs a toplevel task
  on a scheduler. Subtasks will run on the caller domain or on the
...
Similarly to obligations, it can be obtained through \c{async} (\refTirName{async-spec}).
